...
It ships 38 rules across seven layers — from where an error type may be defined
...
38 rules, plus 4 templates the interview instantiates with your own names.
...
fixtures, in its own CI: **34 rules enabled, 34 proven to fire, no findings.**
Four of the 38 are switched off here and three violations are frozen with a
review date, each as a written decision in [`docs/rules.md`](docs/rules.md) —
because `L5.NO_INERT_RULE` refuses to let a rule be enabled and pointed at
nothing.
